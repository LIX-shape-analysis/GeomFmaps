...
class Config:
    ...
    def load(self, path):
        ...
        for line in lines:
            ...
            if len(line_info) > 1 and line_info[0] != '#':
                ...
                elif line_info[0] == 'neig':
                    self.neig = int(float((line_info[2])))

                elif line_info[0] == 'neig_loss':
                    self.neig_full = int(float((line_info[2])))

                elif line_info[0] == 'lambda_Laplacian':
                    if line_info[2] == 'None': self.lam_reg = None
                    else: self.lam_reg = float(line_info[2])
    ...
    def save(self, path):
        ...
        with open(join(path, 'parameters.txt'), "w") as text_file:
            ...
            text_file.write('# FMAP parameters\n')
            text_file.write('# *******************\n\n')
            text_file.write('neig = {:f}\n'.format(self.neig))
            text_file.write('neig_loss = {:f}\n'.format(self.neig_full))
            if self.lam_reg is None:
                text_file.write('lambda_Laplacian = None\n')
            else: text_file.write('lambda_Laplacian = {:f}\n'.format(self.lam_reg))
